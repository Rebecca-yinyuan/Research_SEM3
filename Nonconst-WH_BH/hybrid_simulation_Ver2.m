function hybrid_simulation_Ver2(S0, I0, Lambda, mu, C0, beta0, t_end)

global Sdie Idie Infect_timepoint infect_timelength Q C S I 
global T_poly V_poly t_array

Sdie = []; Idie = []; Infect_timepoint = []; infect_timelength = []; 
Q = []; C =[]; S = []; S(1) = S0; I = []; I(1) = I0; t_array = []; 
t_array(1) = 0;
...
function [t, T, Tstar, V] = Determ_WH(k, c, p, mu_c, delta_c, Lambda, ...
    T0, Tstar0, V0, t_end)
...
[t, T, Tstar, V] = Determ_WH(k, c, p, mu_c, delta_c, Lambda, ...
    T0, Tstar0, V0, t_end); 
