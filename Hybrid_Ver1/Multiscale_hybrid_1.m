function Multiscale_hybrid_1(beta0)
...
[~, ~, T_realtime, infect_timepoint] = CTMC_BH_driver(beta0); 
...
function [S, I, t, infect_timepoint] = CTMC_BH(mu, Lambda, t_end, population, V_SS, beta0)
...
while t(index) <= t_end
    ...
    if infectious_increase == 1
        infect_timepoint(I(1) + index) = t(index + 1); 
    else
...
function plot_SI(S, I, t)
    plot(t, S,'b', t, I, 'k', 'LineWidth', 2); hold on;
end



%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [S, I, t, infect_timepoint] = CTMC_BH_driver(beta0)

% Initialise the parameters for the Between-Host system:
Lambda = 10 ^ (-5);
mu = 10 ^ (-5);
%t_end = 300; population = 10 ^ (5);
population = 10 ^ (2); t_end = 20;
[~, ~, V_SS] = const_WH();

[S, I, t, infect_timepoint] = CTMC_BH(mu, Lambda, t_end, population, V_SS, beta0);
