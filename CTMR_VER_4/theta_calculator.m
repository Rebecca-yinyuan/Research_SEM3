function theta_calculator(t_index, Lambda, mu)

global I S V_poly T_poly infect_timelength V_SS T_SS Theta V_AVE T_AVE t_array
...
V_ave = V_sum / (I(length(I)) + S(length(S))); V_AVE(t_index + 1) = V_ave;
T_ave = T_sum / (I(length(I)) + S(length(S))); T_AVE(t_index + 1) = T_ave;
